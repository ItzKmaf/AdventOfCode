...
public class CoordManager {
    ...
    public boolean isRegisteredCoord(int row, int column) {
        // If the row hasn't been initialised yet, there can be no coords so return false.
        if (possiblePartChords[row] == null) {
            return false;
        }
        return possiblePartChords[row].contains(column);
    }
}
